...
const Supply = ({ values }) => {
  ...
  return (
    <Card className="h-md-100">
      <FalconCardHeader className="pb-0" title="Supply" light={false} titleTag="h5" />
      ...
      <div className="fs-4 font-weight-normal text-sans-serif text-700 line-height-1 mb-1">
              {currency}
              <span className="mx-2">{numberFormatter(supply.usd, 2)}</span>
            </div>
            <Badge pill color="soft-success" className="fs--1 mt-2">
              <img src={ethIcon} alt="eth-icon" style={{width:'20px'}}/>
              <span className="mx-2">{numberFormatter(supply.eth, 2)}</span>
            </Badge>  
            </Col>    
      </Row>
      </CardBody>
